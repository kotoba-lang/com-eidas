(ns eidas.main
  "Kotodama WASM entrypoint for the eidas clean-room actor (L4) — Clojure port.
  ...
  facts (`eidas.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "AuthenticationAssertion" :plural "authenticationassertions" :id-prefix "eidas_aut"
    :fields [:levelOfAssurance :issueInstant :sessionIndex :issuer] :required [:levelOfAssurance]
    :coerce {}                       :refs {}}
   {:entity "EidasAttribute" :plural "eidasattributes" :id-prefix "eidas_eid"
    :fields [:name :friendlyName :nameFormat :value] :required [:name :friendlyName]
    :coerce {}                       :refs {}}
   {:entity "TrustService" :plural "trustservices" :id-prefix "eidas_tru"
    :fields [:serviceProvider :serviceName :serviceType :status :statusStartingTime] :required [:serviceProvider]
    :coerce {}                       :refs {}}
   {:entity "Subject" :plural "subjects" :id-prefix "eidas_sub"
    :fields [:nameID :format :spNameQualifier] :required [:nameID :format]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"eidas.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "eidas-compat" :tier tier :entities entities} 200])
